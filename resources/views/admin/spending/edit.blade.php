                ...
                <div class="row ml-2">
                    <div class="col-md-6">
                        ...
                        <div class="card card-primary">
                            ...
                            <!-- form start -->
                            <form action="{{ route('admin.spending.update', $spending->id )}}" method="POST">
                                @method('PATCH')
                                @csrf
                                <div class="card-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="type_id">
                                            @foreach($types as $type)
                                                <option value="{{ $type->id }}" {{ $type->id == $spending->type_id ? ' selected' : '' }}>
                                                    {{ $type->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="select2" name="tag_ids[]" multiple="multiple"
                                            ...
                                            @foreach ($tags as $tag)
                                                <option value="{{$tag->id}}" {{ is_array ( $spending->tags->pluck('id')->toArray() ) && in_array($tag->id, $spending->tags->pluck('id')->toArray() ) ? ' selected' : ''}}>
                                                    {{$tag->title}}
                                                </option>
                                            @endforeach
